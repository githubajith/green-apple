 ...
 *
 */
package com.mystore.cockpits.setup;
...
import com.mystore.cockpits.constants.MystoreCockpitsConstants;
 ...
 */
@SystemSetup(extension = MystoreCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(MystoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(MystoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						MystoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/mystorecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
